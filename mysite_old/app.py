...
def pchange(ID):
    ...
    sql = f"SELECT hold , status , train_company , eta , amount , size  , Orders.remark ,  address  , delivery_type , lfdwarehouse  , payment_methods  ,  Orders.email , broker ,Broker.company, User.job_id FROM Orders JOIN Broker ON Orders.broker = Broker.broker_id JOIN User ON Orders.operator = User.user_id WHERE order_id = '{ID}' "
